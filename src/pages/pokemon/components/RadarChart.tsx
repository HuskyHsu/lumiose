...
const Text = ({ deg, text, value }: TextPorps) => {
  ...
  const color = (text ?? '').includes('+')
    ? 'fill-red-600 stroke-[0.5px] stroke-red-600'
    : 'fill-slate-600';
...
export function RadarChart({
  ...
  const labels = [
    ...
    { name: 'Attack', value: atk, deg: 330, plus: ev_atk },
    { name: 'Defense', value: def, deg: 30, plus: ev_def },
    { name: 'Speed', value: speed, deg: 90, plus: ev_speed },
    { name: 'Sp.Def', value: spDef, deg: 150, plus: ev_spDef },
    { name: 'Sp.Atk', value: spAtk, deg: 210, plus: ev_spAtk },
  ];
