...
async function mergeLanguageData(zhFile, jaFile, enFile) {
  try {
    // 讀取三個語言版本的資料
    const [zhData, jaData, enData, rowData] = await Promise.all([
    ...
    for (const pid of allPids) {
      ...
      if (basePokemon.alphaMove) {
        mergedPokemon.alphaMove = createMultiLanguageField(
          zhPokemon?.alphaMove,
          jaPokemon?.alphaMove,
          enPokemon?.alphaMove
        );
 ...
 */
function mergeMoveArray(zhMoves, jaMoves, enMoves) {
  const baseMoves = enMoves || jaMoves || zhMoves || [];

  return baseMoves.map((baseMove, index) => {
    const zhMove = zhMoves?.[index];
    const jaMove = jaMoves?.[index];
    const enMove = enMoves?.[index];

    return {
      level: baseMove.level,
      name: createMultiLanguageField(zhMove?.name, jaMove?.name, enMove?.name),
      plus: baseMove.plus,
    };
  });
}
 ...
 */
function mergeTMArray(zhTMs, jaTMs, enTMs) {
  const baseTMs = enTMs || jaTMs || zhTMs || [];

  return baseTMs.map((baseTM, index) => {
    const zhTM = zhTMs?.[index];
    const jaTM = jaTMs?.[index];
    const enTM = enTMs?.[index];

    return {
      tm: baseTM.tm,
      name: createMultiLanguageField(zhTM?.name, jaTM?.name, enTM?.name),
    };
  });
}
